...
Cypress.Commands.add("openAssetsList", () => {
  cy.get('[data-testid="header"] [role="button"]')
    ...
    .get(".MuiDataGrid-row", { timeout: 30000 })
    .should("be.visible");
>>>>>>> 0c2de7c (Added tests to verify assets filter tests)
});
...
Cypress.Commands.add(
  ...
  (name, description, brand, timezone) => {
    cy.get("button").contains("Create New").click();
    cy.get("#name").type(name);
    cy.get("#description").type(description);
    cy.get("#brand").type(brand);
    ...
    cy.get("li").contains(timezone).click();
    cy.get('.MuiGrid-root > [data-testid="global-button-component"]').click();
    cy.get('[data-testid="snackbar-title"]')
      .should("be.visible")
      .contains("Organization Created Successfully!");
  }
);
>>>>>>> 0c2de7c (Added tests to verify assets filter tests)
