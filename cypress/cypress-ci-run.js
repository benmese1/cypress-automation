...
function getEnvNumber(varName, { required = false }, GetEnvOptions = {}) {
...
(async () => {
	try {
		...
		const command = `yarn cypress run --spec "$(yarn --silent ts-node --quiet scripts/cypress-spec-split.js ${totalRunners} ${thisRunner})"`;
