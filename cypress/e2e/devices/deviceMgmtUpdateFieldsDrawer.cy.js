describe('Verify device management table view', () => {
	...
	it('Verify update device rows visibility in raw', () => {
		...
		cy.wait(1000);
		cy.get('[data-id="63863905cb2ab377cdeadeed"] > [data-field="asset_name"]').click();
		...
		cy.get(':nth-child(6) > .MuiFormControl-root > .MuiFormLabel-root').contains('Order #');
		cy.get(':nth-child(7) > .MuiFormControl-root > .MuiFormLabel-root').contains('SIM ID');
		cy.get(':nth-child(8) > .MuiFormControl-root > .MuiFormLabel-root').contains('Packing List');
	});
});
