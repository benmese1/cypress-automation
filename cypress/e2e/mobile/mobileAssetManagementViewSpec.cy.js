describe('Mobile Asset Management View verification', () => {
	...
	const validateDialog = () => {
		cy.get('[data-testid="global-button-component"]').contains('Upload').click();
		cy.get('[role="dialog"]').contains('Upload Assets CSV file').should('be.visible');
		cy.get('[role="dialog"]').get('[data-testid="global-button-component"]').contains('Cancel').click();
	};
});
